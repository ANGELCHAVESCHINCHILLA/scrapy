...
:ref:`Engine <component-engine>`, :ref:`Scheduler <component-scheduler>`,
:ref:`Downloader <component-downloader>`,
:ref:`Downloader Middlewares <component-downloader-middleware>`
and :ref:`Item Pipelines <component-pipelines>`) and the user, that is the one
that starts the process making a request and finish it receiving a response. All
of them have their respectives execution occurrences when each of them have
active participation in the process, and none have a destruction occurrence
because the system works with a cycle of solving requests, so the instances of
the components already exist and each of them keep working for next requests. 
